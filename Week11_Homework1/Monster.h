...
protected:
	...
	int nItem;		// 먹은 아이템 수
	Point q, p;		// 추가: 이전 위치(q)와 현재 위치(p)            9~12
	int nSleep;		// 추가: 쉬어야 하는 횟수
	double dist;		// 추가: 쉬고 난 다음 움직인 거리
	double total;		// 추가: 총 움직인 거리

	void clip(int maxx, int maxy) { p(maxx, maxy); }                //14
	void eat(int** map) {
		if (map[p.y][p.x] == 1) {
			map[p.y][p.x] = 0;
			nItem++;
		}
		dist += (double)(p - q);                                    //20~26
		total += (double)(p - q);
		q = p;
		if (dist > 20) {
			dist = 0;
...
public:
	Monster(string n = "무명괴물", string i = "※", int x = 0, int y = 0)
		...
		p(x, y), q(x, y), nSleep(0), dist(0.0), total(0.0) {}                 //38
	virtual ~Monster() { cout << icon << nItem << "거리:" << total << endl; } //39
